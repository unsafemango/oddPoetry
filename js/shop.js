...
class ShopUI {
    ...
    let books = products
      ...
      <!-- single product -->
        <article class="product">
          ...
          <h3>${product.title.substring(0, 20)}...</a></h3>
          <h4>$${product.price}</h4>
        </article>
  ...
  showPopup() {
    proceedBtn.addEventListener("click", () => {
      alert(
        "Sorry! We are on a holiday break. Please try again after the holidays"
      );
    });
  }
  ...
  addToCart() {
    ...
    btns.forEach((btn) => {
      ...
      btn.addEventListener("click", (e) => {
        ...
        let cartItem = { ...ShopStorage.getProduct(id), amount: 1 };
        // add item to the cart
        cart = [...cart, cartItem];
        // save cart in local storage
        ShopStorage.saveCart(cart);
        // set cart values
